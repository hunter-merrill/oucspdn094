...
__global__ 
void convolution_kernel(int* inputMatrix, int* outputMatrix, int* filterMatrix, int n_row, int n_col)
{
...
__global__ 
void maxpooling_kernel(int* inputMatrix, int* outputMatrix, int n_row, int n_col)
{
    int Col = blockIdx.x * blockDim.x + threadIdx.x;
    int Row = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    if (Col < n_col && Row < n_row) {
        ...
        for (int blurRow = -BLUR_SIZE; blurRow < BLUR_SIZE +1; ++blurRow) {
            for(int blurCol = -BLUR_SIZE; blurCol < BLUR_SIZE+1; ++blurCol) {
                ...
                if( curRow > -1 && curRow < n_row && curCol > -1 && curCol < n_col)
                {
                    if (inputMatrix[curRow*n_col + curCol] > max)
                            max = inputMatrix[curRow*n_col + curCol];
                }
            }
        }

        outputMatrix[Row*n_col+Col] = max;
